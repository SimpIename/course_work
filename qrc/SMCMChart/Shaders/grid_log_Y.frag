...
void main()
{
    ...
    distance_minor_uv.y = min(min(min(abs(fract(scaled_uv.y)-0.301029996),
                              abs(fract(scaled_uv.y)-0.477121255)),
                              min(abs(fract(scaled_uv.y)-0.602059991),
                              abs(fract(scaled_uv.y)-0.698970004))),
                              min(min(abs(fract(scaled_uv.y)-0.77815125),
                              abs(fract(scaled_uv.y)-0.84509804)),
                              min(abs(fract(scaled_uv.y)-0.903089987),
                              abs(fract(scaled_uv.y)-0.954242509)))) / fwidth(scaled_uv.y);

    distance_minor_uv.x = abs(fract(scaled_uv.x)-0.5) / fwidth(scaled_uv.x);
    float dash_length = 8;
...
//    float d_axes = min(distance_axes_uv.x, distance_axes_uv.y);
    float d_axes = distance_axes_uv.x;
