...
double calculatorElasticModulus::a_0_(double delta_Theta_temp)
{
    ...
    return exp(-a_coeffElasticProperties * a_coeffElasticProperties * R * R);
}

//Вычисление модуля упругости полимерной фазы
double calculatorElasticModulus::Elast_mod_polymer_phase_()
{
    Elast_mod_polymer_phase = Elastic_modulus_experiment[0] / (F_(porosity_0));
...
{
    return Elastic_modulus_experiment[j_] / Elast_mod_polymer_phase_();
}
...
{
    return Elast_mod_polymer_phase * F_(porosity)*a_0_(delta_Theta_temp);
}
...
double calculatorElasticModulus::Delta_(double x)
{
    ...
    Elast_mod_polymer_phase = Elast_mod_polymer_phase_();
    a_coeffElasticProperties = x;
...
void calculatorElasticModulus::goldenratio_a()
{
    ...
    a_coeffElasticProperties = (a_ + b_) / 2;
}

//Функция под I
double calculatorElasticModulus::Fun_(double t_i)
{
    return exp(-Energy_activThermDestruct / (temperature_0 + heating_rate * t_i));
}
...
void calculatorElasticModulus::Density_polymer_phase_f()
{
    density_polymer_phase = density_experiment_0 / (1 - porosity_0);
}

//Отношение плотностей (ро_m/ро_m^0)
double calculatorElasticModulus::density_ratio(double delta_Theta_temp)
{
    return 1 - gas_coef + gas_coef * exp(-(preExponental_factor / density_polymer_phase)*I_(delta_Theta_temp));
}

double calculatorElasticModulus::Delta_2(double *x)
{
    gas_coef = x[0];//Коэффициент газификации (Г)
    preExponental_factor = x[1];//Предэкспоненциальный множитель (J_0)
    Energy_activThermDestruct = x[2];//Энергия активации процесса термодиструкции (E_A)
...
double calculatorElasticModulus::volConcentration_polymerPhase_(int j_)
{
    ...
    for (int i = 0; i < j_ - 1; i++) {
        ...
        fi_b_temp = fi_b_temp * (1 / (1 + (delta_t / density_polymer_phase)*preExponental_factor*exp(-Energy_activThermDestruct / (delta_Theta[i] + temperature_0))));
    }
    return fi_b_temp;
}
...
{
    return ((1 - porosity_0) - volConcentration_polymerPhase)*(density_polymer_phase / density_pyrol_phase)*(1 - gas_coef);
}
...
double calculatorElasticModulus::a_1_fun(double porosity, double vol_concentrat_pyrol_phase, double a_0)
{
    ...
    return 1 / (S_b / (a_0*(S_p*S_p - S_b * S_b) + m_p_ElastModulsRatio * (1 - S_p * S_p)) + (S_p - S_b) / (a_0*S_p*S_p + m_p_ElastModulsRatio * (1 - S_p * S_p)) + (1 - S_p));
}
...
double calculatorElasticModulus::fun(double porosity, double vol_concentrat_pyrol_phase, double a_0, double a_1, double x) {
    ...
    double S_b = S_b_(porosity);
    double S_p = S_p_(vol_concentrat_pyrol_phase);
    m_p_ElastModulsRatio = x;

    return S_b / (a_0*(S_p*S_p - S_b * S_b) + m_p_ElastModulsRatio * (1 - S_p * S_p)) + (S_p - S_b) / (a_0*S_p*S_p + m_p_ElastModulsRatio * (1 - S_p * S_p)) + (1 - S_p) / m_p_ElastModulsRatio - 1 / a_1;
}
...
//Твердая полимерная фаза фи_b для температуры teta_max
    ...
    if (fun(porosity, vol_concentrat_pyrol_phase, a_0_max, a_1_max, a_ot) != 0) {
        ...
        b_ot = a_ot;
    m_p_ElastModulsRatio = (a_ot + b_ot) / 2.0;
}
...
QVector<double> calculatorElasticModulus::calculationDensity()
{
    ...
    Elast_mod_polymer_phase = Elast_mod_polymer_phase_(); //Elast_mod_polymer_phase
    goldenratio_a(); //a_coeffElasticProperties
    Density_polymer_phase_f();// density_polymer_phase
    variable_descent_method(); //gas_coef, preExponental_factor, Energy_activThermDestruct

    a_1_max = a_1_(Num_points - 1);
    a_0_max = a_0_(delta_Theta[Num_points - 1]);

    bisection_method(); //m_p_ElastModulsRatio
...
{
    calculationDensity();
    qDebug()<<"EM badabum one";
    //Модуль упругости полимерной фазы
    double Elast_mod_polymer_phase = Elast_mod_polymer_phase_();
    ...
    for (int i = 0; i < Num_points; i++) {
        if (i < j) {
            ...
            Elastic_modulus_[i] = Elast_mod_polymer_phase * a_1_temp;
        }
    }
    qDebug()<<"EM badabum two";
    return Elastic_modulus_;
}

double calculatorElasticModulus::get_Elast_mod_polymer_phase()
{
    return Elast_mod_polymer_phase;
}

double calculatorElasticModulus::get_a_coeffElasticProperties()
{
    return a_coeffElasticProperties;
}

double calculatorElasticModulus::get_density_polymer_phase()
{
    return density_polymer_phase;
}

double calculatorElasticModulus::get_gas_coef()
{
    return gas_coef;
}

double calculatorElasticModulus::get_preExponental_factor()
{
    return preExponental_factor;
}

double calculatorElasticModulus::get_Energy_activThermDestruct()
{
    return Energy_activThermDestruct;
}
...
double calculatorElasticModulus::get_m_p_ElastModulsRatio()
{
    return m_p_ElastModulsRatio;
}

double calculatorElasticModulus::get_Delta_1() {
    return Delta_(a_coeffElasticProperties);
}
double calculatorElasticModulus::get_Delta_2() {
    double x[3];
    x[0] = gas_coef;
    x[1] = preExponental_factor;
    x[2] = Energy_activThermDestruct;
    return Delta_2(x);
}
